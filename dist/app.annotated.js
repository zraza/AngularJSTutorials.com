...
angular.module('zanbeelx', [])
    ...
    .factory('youtubePlayer', ["$window", "$rootScope", "$log", function($window, $rootScope, $log) {
        var _api = {};
        var _ready = false;
        var _objPlayer = null;
...
angular.module('app.default').directive('itemList',
   function() {
      return {
         ...
         link: function(scope, element, attrs) {},
         controller: ["$scope", "Slug", function($scope, Slug) {
...
angular.module('app.default')
	.controller('DetailCtrl', ["$scope", "$http", "Slug", function($scope,$http,Slug) {
		'use strict';
...
angular.module('app', [
   ...
   'slugifier',
   'ngAnimate'
]).config(["$locationProvider", function($locationProvider /*, $provide*/ ) {
   'use strict';
